...
class ComfyConnector:
    ...
    _instance = None
    _process = None
    initialized = False  # The 'initialized' flag tracks if the expensive setup has been run.
    ...
    def __new__(cls, *args, **kwargs):
        ...
        """
        print("ComfyConnector __new__: Creating new instance")
    ...
    def __init__(self, config_path=None):
        ...
        """
        print("ComfyConnector __init__: Setting up synchronous assignments")
    ...
    async def create(cls, config_path=None):
        ...
        Use this instead of direct instantiation for async initialization.
        """
        print("ComfyConnector create: Creating new instance")
    ...
    async def _ensure_initialized(self):
        ...
        """
        print("ComfyConnector _ensure_initialized: Ensuring initialization...")
        # The async lock prevents multiple concurrent calls from trying to initialize at the same time.
        async with self._init_lock:
            if self.initialized:
                print("ComfyConnector _ensure_initialized: singleton already initialized, skipping initialization")
                return

            print("ComfyConnector _ensure_initialized: Initializing ComfyConnector for the first time...")
            ...
            print("ComfyConnector _ensure_initialized: Initializing headless browser with Async Playwright...")
            ...
            try:
                ...
                print("ComfyConnector _ensure_initialized: Playwright and headless Chromium initialized successfully.")
            except Exception as e:
                if "Executable doesn't exist" in str(e) or "missing dependencies" in str(e):
                    print("ComfyConnector _ensure_initialized: --- ONE-TIME SETUP: PLAYWRIGHT BROWSER NOT FOUND ---")
                    print("ComfyConnector _ensure_initialized: Attempting to install Chromium now. This may take a few minutes...")
                    try:
                        ...
                        print("ComfyConnector _ensure_initialized: --- BROWSER INSTALLATION COMPLETE ---")
                        ComfyConnector._browser = await ComfyConnector.playwright.chromium.launch(headless=True)
                        print("ComfyConnector _ensure_initialized: Playwright and headless Chromium re-initialized successfully.")
            ...
            print("ComfyConnector _ensure_initialized: Setting up ComfyUI API process...")
            await self._start_api()            

            self.initialized = True  # Tentatively set before releasing lock

        # Outside lock: Run the test (avoids reentrancy deadlock)
        print("ComfyConnector _ensure_initialized: Running test workflow to ensure server is fully operational...")
        test_passed = False
        try:
            ...
            print(f"ComfyConnector _ensure_initialized: Test failed with error: {e}")
            raise
        finally:
            if not test_passed:
                async with self._init_lock:
        ...
        print("ComfyConnector _ensure_initialized: initialization complete.")
    ...
    def _find_available_port(self):
        ...
        while True:
            try:
                ...
                print(f"ComfyConnector _find_available_port: Server will be hosted in port {port}.")
    ...
    async def _start_api(self):
        ...
        """
        print("ComfyConnector _start_api: Starting ComfyUI API process...")
        if await self._is_api_running():
            print("ComfyConnector _start_api: API is already running.")
            return
        ...
        if self._process is None or self._process.poll() is not None:
            ...
            print(f"ComfyConnector _start_api: API process started with PID: {self._process.pid}")
            ...
            while not await self._is_api_running():
                ...
                attempts += 1
            print(f"ComfyConnector _start_api: API is responsive on port {self.urlport} (PID: {self._process.pid}).")
    ...
    async def _get_prompt_from_workflow(self, workflow: Dict[str, Any]) -> Dict[str, Any]:
        ...
        """
        print("ComfyConnector _get_prompt_from_workflow: Converting workflow to prompt...")
        await self._ensure_initialized()  # Make sure the browser page is ready.

        # Navigate to the page and wait for network idle to ensure resources load.
        await ComfyConnector._page.goto(self.server_address)
        await ComfyConnector._page.wait_for_load_state('networkidle', timeout=30000)
        await ComfyConnector._page.wait_for_selector('#graph-canvas', timeout=20000)
        
        # Wait until window.app is defined (handles timing issues in headless mode).
        await ComfyConnector._page.wait_for_function("() => typeof window.app !== 'undefined'", timeout=30000)
        print("ComfyConnector _get_prompt_from_workflow: window.app is defined.")

        # Execute JavaScript in the page context to transform the workflow.
        await ComfyConnector._page.evaluate("(workflow) => window.app.loadGraphData(workflow)", workflow)
        ...
        """)
        print(f"ComfyConnector _get_prompt_from_workflow: Raw prompt from JS evaluation: {prompt}")
        print("ComfyConnector _get_prompt_from_workflow: Successfully converted workflow to prompt.")
        return prompt
    ...
    async def _test_server(self):
        ...
        """
        print("ComfyConnector _test_server: Testing server...")
        try:
            with open(self.test_payload_file, 'r') as f:
                test_workflow = json.load(f)
            print(f"ComfyConnector _test_server: Loaded test workflow: {test_workflow}")
            test_prompt = await self._get_prompt_from_workflow(test_workflow)
            print(f"ComfyConnector _test_server: Generated test prompt: {test_prompt}")
            ...
            print(f"ComfyConnector _test_server: Error during server test - {e}")
            return False
    ...
    async def _is_api_running(self):
        ...
        """
        print("ComfyConnector _is_api_running: Checking if server is running...")
        try:
    ...
    async def kill_api(self):
        ...
        if self._killed:
            print("kill_api: Already killed, skipping.")
            return
        self._killed = True
